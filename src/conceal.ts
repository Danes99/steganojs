...
import { encrypt, getShasumData } from './encryption';
...
export const conceal = (image: Buffer, message: string | Buffer, encoding?: BufferEncoding, password?: string) => {
  const messageBuffer: Buffer = Buffer.isBuffer(message) ? message : Buffer.from(message, encoding);
  const secretBuffer: Buffer = password ? encrypt(messageBuffer, password) : messageBuffer;
